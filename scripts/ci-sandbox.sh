...
STATUS_CONTEXT="${STATUS_CONTEXT:-dispatch-ci/full-suite}"
...
repo_root() {
  git -C "$ROOT" rev-parse --is-inside-work-tree >/dev/null 2>&1 || die "run from the Dispatch git worktree"
}
...
Creating it makes the full Dispatch tree PUBLIC. That is the deliberate trade
...
Create it once, as the repo owner:
  ...
  gh repo create $CI_REPO --public \\
    --description "Public CI sandbox for Dispatch — full tree, GitHub Actions run here for free. Not a product mirror; feature branches are ephemeral."
...
cmd_prove() {
  ...
  if [ "$result" -eq 0 ]; then
    set_canonical_status "$sha" "success" "Full Dispatch CI passed for exact SHA ${sha:0:12}" "$target_url"
...
The full Dispatch gate ran on [\`${CI_REPO}\`](https://github.com/${CI_REPO}/actions?query=branch%3A${branch}) for this exact SHA before this PR was opened on \`${CANONICAL_REPO}\`.
